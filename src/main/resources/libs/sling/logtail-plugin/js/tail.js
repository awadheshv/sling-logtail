...
// var tailUrl = '/bin/logtail'; // set by servlet in <head>
var logarea;
var follow = true;
var searchCriteria = [];
searchCriteria.push({
...
var showLine = function (text) {
  ...
  for (var i = 0; i < searchCriteria.length; i++) {
    if (text.indexOf(searchCriteria[i]["string"]) >= 0) {
      if (searchCriteria[i]["bold"]) {
        text = "<b>" + text + "</b>";
      }
      if (searchCriteria[i]["italic"]) {
        text = "<i>" + text + "</i>";
      }
      var color = "red";
      if (searchCriteria[i]["forecolor"]) {
        color = searchCriteria[i]["forecolor"];
      }
      var bg = "";
      if (searchCriteria[i]["backcolor"]) {
        bg = searchCriteria[i]["backcolor"];
      }
  ...
  $.ajax({
    ...
    success: function (s) {
      if (s.content) {
        ...
        if (follow) {
          $("html,body")
            .scrollTop(logarea[0].scrollHeight);
        }
      } else {
        $(".loadingstatus")
...
$(document)
  .ready(function (e) {
    ...
    if ($("#highlighting")
      ...
      $("#highlighting")
        ...
        .click(function (e) {
          ...
          $("#highlighting")
            .find(".criteria-list")
            .append("<li class='criteria-item' data-index='" + index + "'><span class='box' style='color:" + color + ";background-color:" + bg + ";font:" + (b ? " bold " : "") + (i ? " italic " : "") + ";'>" + val + "</span>" + val + "</li>");
          $("#highlighting")
            .find("#search")
            .val("");
          $("#highlighting")
            .find("#bold")[0].checked = false;
          $("#highlighting")
            .find("#italic")[0].checked = false;
        });

      $("#highlighting")
        .find(".delete")
        .click(function (e) {
          var $selected = $("#highlighting")
            .find(".criteria-item.selected");
      ...
      $("#highlighting")
        .on("click", ".criteria-item", function (e) {
          $(e.target)
    ...
    $("#speed")
      .change(function (e) {
        refreshInterval = parseInt($(e.target)
          .val());
      ...
      .click(function () {
        if ($(".header")
          ...
          $(".header")
            .slideDown();
          $(".pulldown")
            .attr("title", "Click to hide options");
        }
      });
    ...
    setInterval(function () {
      ...
      $(".loadingstatus")
        .find("li")
        .html("<span style='border-radius:10px;'>" + status + "</span>");

      if (status == "active") {
        $(".loadingstatus")
          .fadeTo(1000, statusOpacity);
    ...
    $("#logfiles")
      .change(function () {
        var selected = $("#logfiles")
          .val();
        if (selected != "") {
          _load = false;
          sendCmd("file:" + selected, function () {
            _load = true;
          });
        }
      });
